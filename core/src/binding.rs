...
extern "C" {
    ...
    fn _sys_write(source: *const u8, offset: u32, length: u32);
    fn _sys_read(target: *mut u8, offset: u32, length: u32) -> u32;
}

#[inline(always)]
pub fn sys_write(source: *const u8, offset: u32, len: u32) {
    unsafe { _sys_write(source, offset, len) }
}

#[inline(always)]
pub fn sys_write_slice(source: &[u8], offset: u32) {
    unsafe { _sys_write(source.as_ptr(), offset, source.len() as u32) }
}
