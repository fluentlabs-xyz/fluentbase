...
pub struct FluentP2PConfig {
    ...
    /// What we tell peers to dial (safe-to-desync — per-operator). Either an
    /// `Ingress::Socket` (IP) or an `Ingress::Dns` hostname; a hostname is
    /// resolved dial-time by peers, and the resolved IP is re-filtered against
    /// `allow_private_ips` post-resolution (commonware `Ingress::resolve_filtered`).
    /// `Ingress::Dns` is only accepted because `ALLOW_DNS = true` (see
    /// [`constants::ALLOW_DNS`]) — a NETWORK-WIDE-SYNCHRONIZED invariant: every
    /// node's `InfoVerifier` drops DNS-form Info when its `allow_dns` is `false`,
    /// so a mixed old/new network partitions on hostname records (upgrade all
    /// validators together before advertising a hostname here).
...
impl FluentP2PConfig {
    ...
    pub fn into_commonware_config(self) -> CommonwareConfig<PrivateKey> {
        ...
        CommonwareConfig {
            ...
            allow_dns: constants::ALLOW_DNS, // network-wide invariant (constants::ALLOW_DNS = true)
