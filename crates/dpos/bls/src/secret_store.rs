...
//! boundary. The per-epoch DKG share has a different shape (a variable-length
//! `(DkgOutcome, Share)` tuple) and its own AEAD-at-rest codec in
//! `consensus/beacon/share_state.rs`; it does not route through this module.
...
impl SecretBackend<'_> {
    ...
    pub fn open(&self, path: &Path) -> Result<Zeroizing<Vec<u8>>, Error> {
        // Refuse a group/other-accessible secret file (a backup/rsync that
        // recreated it at 0644) instead of silently loading it.
...
/// Reject a secret file that is group/other-accessible (`mode & 0o077`), so a key
/// or share directory restored at 0644 fails loud at load instead of running with a
/// leaked secret. Non-unix is a no-op (no POSIX mode). Shared by the validator
/// secret loader and the DKG-share loader (`consensus/beacon/share_state.rs`).
#[cfg(unix)]
...
/// Where the atomic write stages its bytes: a sibling of the target, because
/// `rename` is atomic only within one filesystem (a cross-mount rename fails
/// `EXDEV`). The `.tmp` suffix is outside every reader's filename pattern
/// (`beacon-share-e<E>.bin`, `beacon-dkgjournal-e<E>.bin`), so a leftover from a
/// crashed write is ignored rather than half-loaded, and the next write to the
/// same path reuses it.
...
/// truncate and the flush — for these files a key a node cannot load, i.e. a
/// validator that cannot rejoin. `sync_all` before the rename is what makes the
/// whole-old-or-whole-new guarantee survive a power loss, not just a process crash.
#[cfg(unix)]
pub fn write_mode_0600(path: &Path, data: &[u8]) -> std::io::Result<()> {
    ...
    let mut file = std::fs::OpenOptions::new()
        ...
        .mode(0o600)
        .open(&staging)?;
    // `OpenOptions::mode` only applies at creation: a leftover staging file (or
    // one restored at 0644) keeps its loose mode through the rename, so tighten it
    // explicitly.
...
/// loss and not merely the file's contents. Best-effort: a directory that cannot be
/// opened or synced does not fail the write, since the bytes are already durable.
...
/// written one at a time (an `O_APPEND` open avoids re-reading the whole file per
/// record).
#[cfg(unix)]
pub fn append_mode_0600(path: &Path, data: &[u8]) -> std::io::Result<()> {
    ...
    let mut file = std::fs::OpenOptions::new()
        ...
        .mode(0o600)
        .open(path)?;
    // `OpenOptions::mode` only applies at creation: tighten a pre-existing
    // loose-mode journal file explicitly.
    file.set_permissions(std::fs::Permissions::from_mode(0o600))?;
    file.write_all(data)?;
    // fsync each record before returning: `write_all` only reaches the page cache,
    // so a crash before the OS flushes loses the tail. The journal loader tolerates
    // a torn tail, but syncing minimizes lost dealings and so unnecessary sit-outs
    // on restart.
    file.sync_all()
}
...
mod tests {
    ...
    // A secret restored at 0644 must end at 0600 whichever helper writes it next:
    // `append_mode_0600` re-tightens in place, `write_mode_0600` renames a 0600
    // staging file over the loose target.
    #[test]
