    ...
    R: StakingStateRead + Send + Sync + 'static,
{
    ...
    async fn handle(&mut self, activity: Message) -> Result<(), HandleError> {
        ...
        // Local pre-submit crypto verify — ALWAYS vote-only (bug 4). On-chain
        // `_slashEquivocation` accepts evidence over the ATTRIBUTABLE VOTE HALF
        // only (the threshold seed partial is non-attributable and dropped —
        ...
        // `_ => combined.seed.is_none()` arm rejects a present seed. Vote-only
        // adds nothing for submission even when the full scheme is available (the
        // seed never reaches the chain) and matches the committee stale-fallback
        // window (schemes prune ~8 deep, committees survive `undelegatePeriod + 8`).
