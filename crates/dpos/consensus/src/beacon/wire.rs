...
// Wire: tag(u8) ‖ len(u32) ‖ bytes. An unknown tag decodes to `Err` (forward-safe:
// a future variant takes a new tag; old readers reject rather than misparse).
impl Write for BeaconMessage {
    fn write(&self, buf: &mut impl BufMut) {
        let BeaconMessage::Dkg(bytes) = self;
        TAG_DKG.write(buf);
...
impl EncodeSize for BeaconMessage {
    fn encode_size(&self) -> usize {
        let BeaconMessage::Dkg(bytes) = self;
        size_of::<u8>() /* tag */ + size_of::<u32>() /* len prefix */ + bytes.len()
    }
}
...
impl Read for BeaconMessage {
    ...
    fn read_cfg(buf: &mut impl Buf, _: &Self::Cfg) -> Result<Self, commonware_codec::Error> {
        ...
        // All tagged bodies are a `u32`-len-prefixed opaque blob; cap per tag.
        let cap = match tag {
            TAG_DKG => MAX_DKG_MSG_SIZE,
            _ => {
        ...
        if len > cap {
            return Err(commonware_codec::Error::Invalid(
                "beacon_wire",
                "beacon message body exceeds its cap",
            ));
        }
        ...
        Ok(match tag {
            TAG_DKG => BeaconMessage::Dkg(body),
