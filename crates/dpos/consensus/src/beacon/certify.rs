...
//! the REAL key, but the block ASSERTS a possibly-different `beacon_outcome` key
//! that becomes authoritative on-chain. This gate closes exactly that gap: on a
...
fn seed_certify_verdict(
    ...
    // reachability"; commonware voter contract). If the reporter fan-out is ever reordered so
    // the record can land AFTER the certify scan, this branch becomes a per-node nullify-storm,
    // not a safe verdict. Do not weaken the ordering without re-proving it here.
