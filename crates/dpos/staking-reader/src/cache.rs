...
//! `commonware_runtime::tokio` context rooted at `<datadir>/dpos/` (partitions
//! `vsc-key`/`vsc-val`) and injects it. Exactly ONE instance per process — built
//! by the always-on beacon plane, shared via `SharedBeaconPlane.cache`; a second
//! independent handle over the same partitions is a dual-writer (one prunes a
//! section the other still references → `BlobMissing` on `prune`).
//!
