...
pub struct Config<B, PB, BE, AB, Attrs> {
    ...
    pub blocker: B,
    pub chain_id: u64,
    /// Single cross-epoch `FixedEpocher` — built once in
    /// `OuterBuilder::build`, cloned into both the marshal Config and
    /// every `EpochEngineConfig` constructed in `enter()`.
    ...
    Attrs: Clone + Send + Sync + 'static,
{
        ...
        RR: Receiver<PublicKey = PublicKey>,
    {
        ...
        let (mux_vote, mut vote_mux) = Muxer::new(
        ...
        RR: Receiver<PublicKey = PublicKey>,
    {
        ...
        let vote_sub = match vote_mux.register(epoch.get()).await {
            Ok(s) => s,
            Err(e) => {
                warn!(?epoch, ?e, "skipping epoch enter — vote muxer register failed");
                return;
            }
        };
        let cert_sub = match cert_mux.register(epoch.get()).await {
            Ok(s) => s,
            Err(e) => {
                warn!(?epoch, ?e, "skipping epoch enter — cert muxer register failed");
                return;
            }
        };
        let res_sub = match res_mux.register(epoch.get()).await {
            Ok(s) => s,
            Err(e) => {
                warn!(?epoch, ?e, "skipping epoch enter — res muxer register failed");
                return;
            }
        };
