...
impl<DB: Database> State<DB> {
    ...
    pub fn load_cache_account(&mut self, address: Address) -> Result<&mut CacheAccount, ExitCode> {
...
impl<DB: Database> Database for State<DB> {
        ...
        if let Some(account) = self.cache.accounts.get_mut(&address) {
            ...
            Ok(account
                ...
                .map(|account| match account.storage.entry(index) {
                    hash_map::Entry::Occupied(entry) => Ok::<U256, ExitCode>(*entry.get()),
                    hash_map::Entry::Vacant(entry) => {
    ...
    fn block_hash(&mut self, number: U256) -> Result<B256, Self::Error> {
        // block number is never bigger than u64::MAX.
