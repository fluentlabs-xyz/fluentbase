...
use fluentbase_types::{Address, Bytes, EmptyJournalTrie};
use revm_primitives::{hex, CreateScheme, Env, TransactTo};

type DefaultEvmBuilder = EvmBuilder<'static, SetGenericStage, (), EmptyJournalTrie>;
...
fn test_genesis_greeting() {
    ...
    let mut evm = DefaultEvmBuilder::default().with_env(Box::new(env)).build();
