use crate::interpreter::SStoreResult;
...
use core::mem;
use fluentbase_types::{ExitCode, B256, F254};
...
impl JournaledState {
    ...
    pub fn set_code(&mut self, address: Address, code: Bytecode, code_hash: Option<B256>) {
        ...
        debug_assert_eq!(account.info.code_hash, code.hash_slow());
    ...
    pub fn transfer<DB: Database>(
        ...
        balance: U256,
        db: &mut DB,
    ) -> Result<Option<InstructionResult>, EVMError<ExitCode>> {
        // load accounts
        ...
        let Some(from_balance_incr) = from_balance.checked_sub(balance) else {
            return Ok(Some(InstructionResult::InsufficientBalance));
        };
    ...
    pub fn initial_account_load<DB: Database>(
        ...
        slots: &[U256],
        db: &mut DB,
    ) -> Result<&mut Account, EVMError<ExitCode>> {
        // load or get account.
