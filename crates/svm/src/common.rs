...
use crate::builtins::SyscallAbort;
use crate::builtins::SyscallCreateProgramAddress;
use crate::builtins::SyscallHash;
use crate::builtins::SyscallLog;
use crate::builtins::SyscallMemcmp;
use crate::builtins::SyscallMemcpy;
use crate::builtins::SyscallMemmove;
use crate::builtins::SyscallMemset;
use crate::builtins::SyscallPanic;
use crate::builtins::SyscallPoseidon;
// use crate::builtins::SyscallSecp256k1Recover;
use crate::builtins::SyscallTryFindProgramAddress;
use crate::compute_budget::ComputeBudget;
use crate::context::{InstructionContext, InvokeContext, TransactionContext};
use crate::error::InstructionError;
use crate::feature_set::{blake3_syscall_enabled, bpf_account_data_direct_mapping, enable_poseidon_syscall, error_on_syscall_bpf_function_hash_collisions, reject_callx_r10, switch_to_new_elf_parser, FeatureSet};
use crate::loaded_programs::{LoadedProgram, DELAY_VISIBILITY_SLOT_OFFSET};
use alloc::boxed::Box;
use alloc::sync::Arc;
use alloc::vec;
use alloc::vec::Vec;
...
use solana_program::{blake3, bpf_loader, bpf_loader_deprecated, bpf_loader_upgradeable, hash::{Hash, Hasher}};
...
use crate::fluentbase_helpers::SvmError;
use crate::types::SVM_ADDRESS_PREFIX;
#[cfg(test)]
...
pub fn create_program_runtime_environment_v1<'a, SDK: SharedAPI>(
    ...
    // Keccak256
    result.register_function_hashed(*b"sol_keccak256", SyscallHash::vm::<SDK, Keccak256Hasher<SDK>>)?;
...
pub fn evm_address_from_pubkey<const VALIDATE_PREFIX: bool>(value: Pubkey) -> Result<Address, SvmError> {
    if VALIDATE_PREFIX && &value.as_ref()[12..] != &SVM_ADDRESS_PREFIX {
        return Err(SvmError::ExitCode(ExitCode::Err));
    }
    Ok(Address::from_slice(&value.as_ref()[SVM_ADDRESS_PREFIX.len()..]))
}
...
pub fn lamports_from_evm_balance(value: U256) -> u64 {
    ...
    let bytes: [u8; SIZE_OF_U64] = value.to_be_bytes::<{ U256::BYTES }>().as_ref()[U256::BYTES - SIZE_OF_U64..U256::BYTES].try_into().unwrap();
