...
pub mod tests {
    ...
    use crate::helpers::{calculate_heap_cost, create_account_shared_data_for_test, test_utils};
    use crate::loaded_programs::{LoadedProgram, LoadedProgramType, LoadedProgramsForTxBatch};
    use crate::test_helpers::{load_program_account_from_elf_file, mock_process_instruction, new_test_sdk, prepare_vars_for_tests, process_instruction};
    use crate::{deploy_program, with_mock_invoke_context};
    use alloc::sync::Arc;
    ...
    use solana_program::{bpf_loader, bpf_loader_deprecated, bpf_loader_upgradeable, system_program, sysvar};
    ...
    fn test_bpf_loader_invoke_hello_world() {
        ...
        let program_id = Pubkey::new_unique();
        let mut program_account =
            load_program_account_from_elf_file(&loader_id, "../examples/hello-world/assets/solana_ee_hello_world.so");
        let parameter_id = Pubkey::new_unique();
        let parameter_account = AccountSharedData::new(1, 0, &loader_id);
        let parameter_meta = AccountMeta {
            pubkey: parameter_id,
    ...
    fn test_bpf_loader_upgradeable_upgrade() {
        ...
        let accounts = process_instruction(&sdk, transaction_accounts, instruction_accounts, Ok(()));
