use crate::common::{common_close_account, limited_deserialize, write_program_data, DEFAULT_LOADER_COMPUTE_UNITS, DEPRECATED_LOADER_COMPUTE_UNITS, PACKET_DATA_SIZE};
use crate::context::IndexOfAccount;
use crate::declare_builtin_function;
use crate::error::InstructionError;
use crate::feature_set::{bpf_account_data_direct_mapping, enable_bpf_loader_extend_program_ix, enable_bpf_loader_set_authority_checked_ix};
use crate::loaded_programs::{LoadedProgram, LoadedProgramType};
use crate::sysvar_cache::get_sysvar_with_account_check;
use crate::{context::InvokeContext, create_vm2, deploy_program, native_loader, serialization};
use alloc::boxed::Box;
use alloc::sync::Arc;
use alloc::vec::Vec;
...
use solana_program::{bpf_loader, bpf_loader_deprecated, bpf_loader_upgradeable, entrypoint::SUCCESS, pubkey::{Pubkey, PubkeyError}, system_instruction};
...
fn execute<'a, SDK: SharedAPI>(
    ...
    let execution_result = {
        ...
        create_vm2!(vm, executable.as_ref(), regions, accounts_metadata, invoke_context);
        let mut vm = match vm {
            Ok(info) => info,
            Err(_e) => {
        ...
        let (_compute_units_consumed, result) = vm.execute_program(executable.as_ref(), true/*, !use_jit*/);
        drop(vm);
        // ic_logger_msg!(
        //     log_collector,
...
pub fn process_instruction_inner<SDK: SharedAPI>(
    ...
    if native_loader::check_id(program_account.get_owner()) {
        ...
        let program_id = instruction_context.get_last_program_key(&invoke_context.transaction_context)?;
    ...
    let result: Result<(), Box<dyn core::error::Error>> = match executor_program_ref {
