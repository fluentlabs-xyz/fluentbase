...
use commonware_p2p::{utils::mux::Muxer, Ingress};
        ...
        + 'static,
{
    ...
    let peer_count: std::sync::Arc<dyn Fn() -> usize + Send + Sync> = {
        let net = node.network.clone();
        std::sync::Arc::new(move || net.num_connected_peers())
    };
    ...
    let bcast_mux_handle =
        ctx.with_label("follower_broadcast_mux_sup")
    ...
    // SOLE producer); there is no no-verify mode in v1 — the standalone bare
