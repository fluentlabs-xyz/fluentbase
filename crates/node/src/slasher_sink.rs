...
/// `gas_limit` (and disabling balance/nonce checks for the `signer_address`
/// caller); deferred as it rewrites the tested sim path for a fail-safe edge.
...
/// variants sign via `Signer::sign_hash`.
pub enum SlasherSigner {
    ...
    E: ConfigureEvm + Clone + Send + Sync + 'static,
{
    ...
    fn simulate(&self, target: Address, calldata: Bytes) -> Result<Sim, String> {
        ...
        let out = evm
            .transact_system_call(self.signer_address, target, calldata)
    ...
    E: ConfigureEvm + Clone + Send + Sync + 'static,
{
    fn submit<'a>(
        ...
        Box::pin(async move {
            ...
            let sig = match self.signer.sign_hash(&tx.signature_hash()).await {
                Ok(s) => s,
...
mod tests {
    ...
    #[tokio::test]
    async fn slasher_signer_local_round_trip_recovers_its_own_address() {
        let local = PrivateKeySigner::random();
        let signer = SlasherSigner::Local(local);
        let hash = B256::random();
        let sig = signer.sign_hash(&hash).await.expect("sign_hash");
        let recovered = sig
