...
use fluentbase_consensus::{ExecutedChain, FinalizedCursor, OrderBlock, OrderingAssembler};
...
/// backfill. Carries the executor-advanced [`FinalizedCursor`] so the result
/// gate (propose + verify) samples the FINALIZED tier (reth's canonical chain
/// below the cursor), not the speculative head.
#[derive(Clone, Debug)]
pub struct ProviderExecutedChain<P> {
    provider: P,
    ...
    P: BlockHashReader + BlockNumReader + Clone + Send + Sync + 'static,
{
    fn executed_tip(&self) -> u64 {
        self.provider.last_block_number().unwrap_or(0)
    }

    fn spec_executed_hash(&self, height: u64) -> Option<B256> {
        self.provider.block_hash(height).ok().flatten()
    }

    fn finalized_executed_hash(&self, height: u64) -> Option<B256> {
        // Tier-F IS reth's canonical chain below the finalized-execution cursor:
        // at-or-below the cursor the provider's canonical hash cannot have a
        // sibling (marshal-acked pre-restart / below a BLS-authenticated jump
        // landing / eager-derived past the canonical postcondition), so it IS
        // the finalized hash; above it the height is still speculative-tier ⇒
        // None. Without the cursor seed a committee-wide restart
        // propose-skips/false-votes into a permanent wedge.
        self.finalized
