...
impl PayloadAttrsBuilderLike for FluentPayloadAttributesBuilder {
    ...
    fn payload_id(&self, parent_hash: B256, attrs: &EthPayloadAttributes) -> PayloadId {
        reth_ethereum_engine_primitives::payload_id(&parent_hash, attrs)
    }
}
    ...
    >,
{
    ...
    async fn build_payload_builder(
        ...
        let extra_data = if self.dpos_active {
            if !conf.extra_data_bytes().is_empty() {
                tracing::warn!(
        ...
        > + Clone,
{
    ...
    fn try_build(
        ...
        let cfg = self.config_for(args.config.attributes.id);
    ...
    fn build_empty_payload(
        ...
        let cfg = self.config_for(config.attributes.id);
        let pool = self.pool.clone();
