...
pub fn main() {
    ...
    // let mut input = ExecutionContext::raw_input();
    // let context = ExecutionContext::DEFAULT;
    // let am = JzktAccountManager::default();
    // let mut gas_limit = context.contract_gas_limit() as u32;
    // let method_data = EvmCallMethodInput {
    //     callee: context.contract_address(),
    //     value: context.contract_value(),
    //     input: context.contract_input(),
    //     gas_limit: context.contract_gas_limit(),
    //     depth: 0,
    // };
    // let core_input = CoreInput::new(EVM_CALL_METHOD_ID, method_data);
    // input.contract_input = core_input.encode_to_vec(0).into();
    // input.contract_address = ECL_CONTRACT_ADDRESS;
    // let (ecl_account, _) = am.account(ECL_CONTRACT_ADDRESS);
    // let contract_input_data_vec = input.encode_to_vec(0);
