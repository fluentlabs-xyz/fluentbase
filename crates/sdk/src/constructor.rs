...
fn encode_wasm_custom_section(name: &str, payload: &[u8]) -> Result<Vec<u8>, io::Error> {
    let mut section = Vec::new();
    let mut content_length = Vec::new();
    let mut name_length = Vec::new();

    leb128::write::unsigned(&mut name_length, name.as_bytes().len() as u64)?;
    leb128::write::unsigned(
        &mut content_length,
        (name_length.len() + name.len() + payload.len()) as u64,
    )?;
...
/// necessary data.
pub fn encode_constructor_params(payload: &[u8]) -> Result<Vec<u8>, io::Error> {
...
mod tests {
    ...
    fn test_encode_wasm_input_section() {
        ...
        let result = encode_constructor_params(&payload).expect("Encoding should succeed");
