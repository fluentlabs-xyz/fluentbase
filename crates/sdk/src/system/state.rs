...
use rwasm_core::N_MAX_RECURSION_DEPTH;
...
impl RecoverableState {
    ...
    pub(super) fn recover() -> Self {
        let mut recoverable_state = lock_state_context();
        let Some(state) = recoverable_state.pop() else {
...
pub fn lock_state_context<'a>() -> MutexGuard<'a, Vec<RecoverableState>> {
    static SAVED_STATE_CONTEXT: Once<Mutex<Vec<RecoverableState>>> = Once::new();
    let cached_state =
        SAVED_STATE_CONTEXT.call_once(|| Mutex::new(Vec::with_capacity(N_MAX_RECURSION_DEPTH)));
...
pub(super) fn next_unique_key() -> u32 {
    ...
    // Safety: 1k TPS gives ~50 days of key rotation that seems safe enough, especially since we reset state every transaction
