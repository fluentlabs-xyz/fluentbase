...
impl SyscallSha256Compress {
    ...
    fn fn_handler(mut caller: Caller<'_, RuntimeContext>, arg1: u32, arg2: u32) -> Result<(), Trap>{
        ...
        let mut hx = [0u32; 8];
        for i in 0..8 {
            let  value = u32::from_be_bytes(caller.read_memory(h_ptr + i as u32 * 4, 4)?.try_into().unwrap());
            hx[i] = value;
        }
        ...
        for i in 0..8 {
            caller.write_memory(h_ptr + i as u32 * 4, &hx[i].wrapping_add(v[i]).to_be_bytes())?;
        }
