...
impl SyscallEcrecover {
    ...
    pub fn fn_impl(digest: &B256, sig: &[u8; 64], rec_id: u8) -> [u8; 32] {
        ...
        let public_serialized = public.serialize_uncompressed();
        let mut result = fluentbase_types::keccak256(&public_serialized[1..]);
        result[..12].fill(0);
        result.into()
    }
}
