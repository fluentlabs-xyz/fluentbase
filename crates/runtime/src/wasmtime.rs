...
const FIRST_VALID_CALL_ID: i32 = 1000000000; // Wasmtime call IDs start from 1 billion to distinguish them
                                             // from those generated by the interpreter runtime
...
impl RuntimeState {
    fn new() -> Self {
        Self {
            ...
            free_call_id: FIRST_VALID_CALL_ID,
        }
    }

    fn suspend_executor(&mut self, executor: AsyncExecutor) -> i32 {
        if self.free_call_id == i32::MAX {
            self.free_call_id = FIRST_VALID_CALL_ID; // Wrap value around to prevent non-positive
                                                     // call ids
        }
        let call_id = self.free_call_id;
        self.free_call_id += 1;
        let existing = self.suspended_executors.insert(call_id, executor);
        assert!(existing.is_none());
        assert!(call_id >= FIRST_VALID_CALL_ID);
        call_id
    }
...
pub fn resume(
    ...
    let executor = RUNTIME_STATE
        ...
        .expect("runtime with the given call id should exist");
