...
use fluentbase_types::{ExitCode, ED25519_COMPRESSED_SIZE, ED25519_DECOMPRESSED_SIZE};
...
pub fn syscall_ed25519_decompress_handler(
    ...
    let mut compressed_edwards_y = [0u8; ED25519_COMPRESSED_SIZE];
    ctx.memory_read(
        slice_ptr + ED25519_COMPRESSED_SIZE,
...
pub fn syscall_ed25519_decompress_impl(
    mut compressed_edwards_y: [u8; ED25519_COMPRESSED_SIZE],
    sign: u32,
    ...
    let mut result = [0u8; ED25519_DECOMPRESSED_SIZE];
    result[32..64].copy_from_slice(&compressed_edwards_y);
    // Re-insert sign bit into last bit of Y for CompressedEdwardsY format
    compressed_edwards_y[ED25519_COMPRESSED_SIZE - 1] &= 0b0111_1111;
    compressed_edwards_y[ED25519_COMPRESSED_SIZE - 1] |= (sign as u8) << 7;
