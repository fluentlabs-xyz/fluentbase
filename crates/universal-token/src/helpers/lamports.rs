use crate::types::pubkey::{Pubkey, PUBKEY_BYTES};
...
pub fn pubkey_from_evm_address(value: &Address) -> Pubkey {
    ...
    pk[Address::len_bytes()..].copy_from_slice(value.as_slice());
    Pubkey::new_from_array(pk)
}

pub fn evm_address_from_pubkey(pk: &Pubkey) -> Address {
    Address::from_slice(&pk.as_ref()[Address::len_bytes()..])
}
