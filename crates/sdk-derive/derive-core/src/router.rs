use crate::{abi::artifacts::ContractArtifacts, mode::Mode, route::Route};
...
fn check_fallback(routes: &[Route]) -> bool {
    routes
        .iter()
        .any(|route| route.method().sig.ident == "fallback")
}
...
impl ToTokens for Router {
    fn to_tokens(&self, output: &mut TokenStream2) {
        ...
        let artifacts = ContractArtifacts::new(&struct_name, &abi_array);
        ...
        let abi_content = artifacts.abi_content;
        let interface_content = artifacts.interface_content;
        ...
        output.extend(quote! {
            ...
            pub const #abi_const_name: &str = #abi_content;
            #[doc = "Solidity interface"]
            pub const #interface_const_name: &str = #interface_content;
        });
    }
}
