...
impl StorageSlot {
    ...
    fn can_use_direct_storage(&self) -> bool {
        ...
        // Special check for our FixedBytes base type
    ...
    fn getter_definition(&self) -> TokenStream2 {
        ...
        // Get the appropriate return type for the getter
        ...
        // Determine which storage access method to use
        if self.can_use_direct_storage() {
            // Use DirectStorage for primitive types
    ...
    fn setter_definition(&self) -> TokenStream2 {
        ...
        // Get the appropriate parameter type for the setter
        let output = self.get_output_type();
...
impl Parse for StorageSlot {
    fn parse(input: ParseStream) -> SynResult<Self> {
        ...
        // Extract arguments and output type
        let (args, output) = kind.parse_args();

        // Create slot (slot number will be assigned later)
        let slot = Self {
            slot: 0,
            name,
            kind,
...
impl Parse for Storage {
    fn parse(input: ParseStream) -> SynResult<Self> {
        // Parse a list of storage slots separated by semicolons
        ...
        // Verify we have at least one storage slot
