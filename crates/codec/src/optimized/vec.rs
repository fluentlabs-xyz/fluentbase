    ...
    T: Encoder<B, ALIGN, false, Ctx=EncodingContext>,
{
    ...
    fn encode_header(
        ...
        let size = if T::IS_DYNAMIC { 0 } else { len * T::HEADER_SIZE as u32 };
        let off = if T::IS_DYNAMIC {
    ...
    fn decode(buf: &impl Buf, offset: usize) -> Result<Self, CodecError> {
        ...
        // Read vector header: [len, offset, size]
        let len = read_u32_aligned::<B, ALIGN>(buf, offset)? as usize;
        let data_off = read_u32_aligned::<B, ALIGN>(buf, offset + word)? as usize;
        let data_sz = read_u32_aligned::<B, ALIGN>(buf, offset + word * 2)? as usize;
        ...
        if data_sz != 0 {
            // Static/data case: all elements are packed in tail
            let data_ptr = offset + data_off;
            let mut out = Vec::with_capacity(len);
            for i in 0..len {
                let elem_off = data_ptr + i * align_up::<ALIGN>(T::HEADER_SIZE);
                out.push(T::decode(buf, elem_off)?);
            }
            return Ok(out);
        }

        // Dynamic/nested case: elements are encoded as headers after this header
        let header_ptr = offset + word * HEADER_WORDS;
        let mut out = Vec::with_capacity(len);
        for i in 0..len {
            let elem_header_off = header_ptr + i * T::HEADER_SIZE;
            out.push(T::decode(buf, elem_header_off)?);
        }
        Ok(out)
    }
}
...
mod tests {
    mod compact {
        ...
        use crate::optimized::{encoder::Encoder, utils::tests::assert_codec};
        ...
        fn test_vec_vec_u32_codec() {
            ...
            assert_codec(
                ...
                concat!(
                "02000000", // 2  - len
                "0c000000", // 12 - offset to first element header
                "00000000", // 0  - size (dynamic elements)
                "03000000", // 3  - len (v0)
        ...
        fn test_deep_nested_vec() {
            ...
            assert_codec(
                ...
                "01000000", // 1 element
                "0c000000", // offset to its header (12)
                "00000000", // size (dynamic elements)
                // header for Vec<Vec<u32>>
                "03000000", // 3 sub-vectors
                "0c000000", // offset to first Vec<u32> header (12)
                "00000000", // size (dynamic elements)
    ...
    mod solidity {
            ...
            T: Encoder<BigEndian, 32, true> + PartialEq + std::fmt::Debug + Clone,
        {
            ...
            let encode_result = T::encode(value, &mut buf);
            assert!(
                encode_result.is_ok(),
