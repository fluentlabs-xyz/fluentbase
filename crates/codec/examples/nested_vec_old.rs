...
pub extern "C" fn main() {
    ...
    SolidityABI::<Vec<u32>>::encode(&simple_vec, &mut buf).unwrap();
    let _decoded: Vec<u32> = SolidityABI::<Vec<u32>>::decode(&buf, 0).unwrap();
    buf.clear();

    // Nested Vec<Vec<u32>>
    let nested_vec: Vec<Vec<u32>> = vec![vec![1, 2, 3], vec![4, 5]];
    SolidityABI::<Vec<Vec<u32>>>::encode(&nested_vec, &mut buf).unwrap();
    ...
    SolidityABI::<Vec<Vec<Vec<u32>>>>::encode(&triple_nested, &mut buf).unwrap();
    ...
    SolidityABI::<Vec<Vec<u32>>>::encode(&mixed_sizes, &mut buf).unwrap();
    let _decoded: Vec<Vec<u32>> = SolidityABI::<Vec<Vec<u32>>>::decode(&buf, 0).unwrap();
    buf.clear();
    ...
    // <Vec<Vec<u32>> as Encoder<BigEndian, 32, true, false>>::encode(&example, &mut buf,
    // 0).unwrap(); let _decoded = <Vec<Vec<u32>> as Encoder<BigEndian, 32, true,
    // false>>::decode(&buf, 0).unwrap();
}
