...
#[cfg(test)]
mod tests {
    use crate::engine::executor::execute_wasm;
    use crate::rwasm::compiler::Compiler;
    use crate::rwasm::module::{ReducedModule, ReducedModuleError};
    use crate::{Config, Engine, Linker, Store};
    use alloc::vec::Vec;
...
(module
  ...
  (export "main" (func $main)))
    "#,
        ...
        let trace = reduced_module.trace_binary();
        // execute translated rwasm
        let config = Config::default();
        let engine = Engine::new(&config);
        let module = reduced_module.to_module(&engine).unwrap();
