...
use crate::bootstrap::{STAKING_DPOS_ADDR, STAKING_ECONOMICS_ADDR, STAKING_REWARDS_ADDR};
...
pub struct Artefacts {
    ...
    /// Third DELEGATECALL'd library `Staking` is linked against — the per-epoch
    /// BLEND stipend settlement logic (`settleEpochStipend` / `getEpochRewards`),
    /// extracted to keep `Staking` under EIP-170. Deployed at `STAKING_REWARDS_ADDR`;
    /// `Staking`'s `__$StakingRewards$__` placeholders are linked to it at load.
    pub staking_rewards: ContractArtefact,
    /// BLEND stipend reserve — holds the reward pot, `disburse` gated to the Staking
    /// predeploy. Deployed at `BLEND_RESERVE_ADDR` with the 6-address StakingContext
    /// tuple + `stakingAddr` constructor, then `initialize`d and seeded with BLEND.
    pub blend_reserve: ContractArtefact,
...
pub fn load(dir: &Path) -> eyre::Result<Artefacts> {
    ...
    Ok(Artefacts {
        ...
        staking_rewards: load_one(&dir.join("StakingRewards.json"))?,
        blend_reserve: load_one(&dir.join("BlendReserve.json"))?,
        ...
        liveness_slashing: load_one(&dir.join("ProductionLiveness.json"))?,
