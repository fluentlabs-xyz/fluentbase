"""cases/smoke — the default-stack smoke cases, mirroring the two bash assertion libraries.
    ...
    asserts_follow.py  the FOLLOWER bodies (`case-cert-follow`, `-cert-cascade`, `-tx-cascade`,
                       plus `-cert-keyless`, which has no bash ancestor), the first cases needing
                       services beyond the base compose
    ...
    verdicts_follow.py the pure decision layer of the followers
...
`cert_keyless` is the one entrypoint here with NO bash ancestor: it is FLU-1202's live gate, and
it is the only case in the tree whose PRECONDITION is the subject. Its conclusion ("the late key
took effect") is satisfied by every healthy node on the chain; only the keyless window it insists
on observing first makes the reading mean anything.
...
The FOLLOWERS have neither argument and each runs alone, as its bash original does. They
ADD nodes rather than disturbing the committee, and three of them leave the stack in a state the
next case must not inherit: `cert-follow` leaves a man-in-the-middle poisoning validator-0's
...
soundness claim, not a saved bring-up. `cert-keyless` poisons nothing — it starts one honest
follower and touches none of the overlay's MITM services — and is standalone for the opposite
reason: its subject is the FIRST seconds of a node's life against a live chain, so a chained run
