...
#   committee → cold-restart ALL validators into unified --dpos (committee v0-v4
#   PLANE-NATIVE, no WS hub; the still-Pending v5 keeps the WS escape) → register
#   the EXTERNAL 6th validator via
...
# ── cold-restart all 6 validators + the full-node into the DPoS overlay ─────
...
#
# THE FULL-NODE IS NOT OPTIONAL IN THIS LIST. Without it the container keeps the
# phase-A command — a WS trust-follow block relay with no --cert-follow — and a
# relay re-executes each block through new_payload with no OrderBlock, so it never
# gets PK_E for the boundary block's `commitEpochBeaconKey` system call, skips the
# commit and diverges on state root at every committee change (the overlay's own
# comment above its full-node service, :140-149). It survives only while the
# committee has not changed yet. `pp_bring_up_rotation` (lib.sh:1393) recreates
# `"${PP_VALS[@]}" full-node` for exactly this reason; the shared helper was added
# with it and this case never back-ported the second half.
...
# Gap A proof (task dpos_sync_plane_upstream): the Active committee validators
# run PLANE-NATIVE — NO --dpos.follower-upstream, so the chain converging above
# means they synced + stayed live purely over the consensus p2p plane with NO WS
# hub. Assert against v0's LIVE args (PID 1 is `fluent` via `exec`) so a
# regression that re-adds a WS upstream to a committee validator fails here.
# (v5, the still-Pending external joiner, legitimately keeps the WS escape until
# activation — Gap C pre-Active onboarding is deferred; plan_questions P4.)
V0_CMDLINE=$(docker compose exec -T validator-0 sh -c 'tr "\0" " " </proc/1/cmdline' 2>/dev/null || true)
[[ "$V0_CMDLINE" == *fluent*--dpos* ]] || { echo "FAIL: could not read v0 live cmdline (got: $V0_CMDLINE)"; exit 1; }
[[ "$V0_CMDLINE" != *follower-upstream* ]] \
    || { echo "FAIL: committee validator-0 carries --dpos.follower-upstream — plane de-hub regressed"; exit 1; }
echo "  committee validators plane-native (no WS hub) — Gap A ✓"
...
# reconcile_roles drops its signer engine) and keeps following the chain. It is
# a committee validator (v0-v4) so it is PLANE-NATIVE now (no
# `--dpos.follower-upstream`): still Active ⇒ still in the tracked set ⇒ its
# marshal follows finalizations over the consensus plane, instead of wedging as a
...
# ── liveness ejection: RETIRED with the participation-floor jail ──────────────
# This leg stopped a committee member for a full epoch and asserted it was JAILED and
# then absent from committee[J+3]. Both halves are gone: the jail machinery
# (`Staking.slash`, `_slashValidator`, `jailedBefore`, `readmitExpiredJails`) is deleted,
# and its replacement — `ProductionLiveness`'s temporary exclusion — cannot be exercised
# from a smoke run at all. Two reasons, either sufficient:
#   * the tier ships FLAG-OFF (`ChainConfig._productionLivenessDisabled` is seeded true),
#     so no verdict is evaluated until governance enables it after the pre-enable evidence;
...
#     construction, which is the right default for smoke and fatal for a case that wants
#     to see an exclusion.
# Exercising it is soak work (lower the floor via governance, or raise the interval), and
# the mechanism itself is covered contract-side in `ProductionLiveness.t.sol`. Do not
# "restore" this leg by re-pointing it at `readmitAtEpoch` — it would assert nothing on a
# stack where the tier never fires.
