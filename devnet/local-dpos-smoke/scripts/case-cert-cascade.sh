...
aligned=$(wait_follower_align "$T1_PORT" "$align_target" 240) || {
    # Both readings, as case-cert-catchup does: the follower's tail alone cannot tell
    # "a few blocks behind" from "on a different chain" (case-cert-catchup.sh:170).
    echo "FAIL (smoke-cert-cascade): tier-1 did not align with v0 past $align_target" \
         "(tier-1=$(check_external "$T1_PORT"), v0=$(check_external 8545))"
...
t2_aligned=$(wait_follower_align "$T2_PORT" "$t2_target" 240) || {
    echo "FAIL (smoke-cert-cascade): tier-2 did not align via the tier-1 window past $t2_target" \
         "(tier-2=$(check_external "$T2_PORT"), tier-1=$(check_external "$T1_PORT"), v0=$(check_external 8545))"
