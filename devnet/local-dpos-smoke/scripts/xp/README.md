...
| `agreement_check.py [--stand N]` | every value declared on BOTH sides of the node/contract boundary (R-113..R-119): ABI signatures against `consts.rs` AND the shipped rWasm blob, the numeric literals, the namespace, the two return SHAPES and the three chain-config return WIDTHS, the call signatures the harness itself hands `cast`, and — with a stand — dispatch, committee order and the epoch/commit-horizon formulas. Reads the in-tree `contracts/staking/src` by default; `STAKING_CONTRACT_SRC` points it at another checkout |
