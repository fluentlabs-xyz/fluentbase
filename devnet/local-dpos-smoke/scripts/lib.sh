...
# True iff the validator's most recent graceful shutdown flushed persistence
# ("engine shutdown complete" as the last shutdown-watcher line). `docker compose
# stop` returns the moment the container exits, but the final buffered log line
# can lag a beat before `docker compose logs` surfaces it; poll up to ~15s so a
# read-race false-negative does not trigger a window-blowing reconverge retry.
shutdown_flushed() {
    # A graceful exit (code 0) means the SIGTERM handler ran flush_engine_in_memory
    # to completion ("engine shutdown complete (in-memory blocks persisted)",
    # bins/fluent/src/main.rs); a SIGKILL on the 40s stop-timeout exits 137. Check the
    ...
    # "flush incomplete" even though the node flushed in <1s (diagnosed 2026-06-05;
...
# Graceful-stop the 4 validators, verify each flushed (retry up to 4×, bringing
