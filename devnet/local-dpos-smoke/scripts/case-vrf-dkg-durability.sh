...
# `vol_mutate_beacon <node-idx> <glob> <sh-snippet-using-$f>` — mutate a file under a STOPPED
# validator's beacon dir through a throwaway genesis-init container on the shared /runtime volume.
# HOISTED TO lib.sh: case-vrf-dkg-halt tears two journals with the same recipe, and a second copy
# of the corruption logic is exactly the thing that drifts.
...
# No slash on either victim — best-effort on the runtime-deployed default config (the hard
# control is the stall-then-resume + recovery legs above). With the liveness jail deleted,
# `ValidatorSlashed` survives ONLY as an equivocation event, which is exactly what a torn
# resume must not trigger; a jailed status likewise has equivocation as its only producer.
slash_events=$(docker compose logs 2>/dev/null | strip_ansi | grep -iE "ValidatorSlashed|equivocat" || true)
for i in 3 4; do
    addr=$(pp_owner_addr "$i")
    hit=$(grep -i "${addr#0x}" <<<"$slash_events" || true)
    [[ -z "$hit" ]] || { echo "FAIL (smoke-vrf-dkg-durability): v$i was slashed despite recovering its share:"; printf '%s\n' "$hit" | sed 's/^/    /'; exit 1; }
