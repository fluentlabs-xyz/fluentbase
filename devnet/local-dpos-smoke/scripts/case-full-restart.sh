#!/usr/bin/env bash
# smoke-full-restart: stop ALL 4 validators (verify each flushed), restart them, and
...
for v in "${VALS[@]}"; do
    shutdown_flushed "$v" || { echo "FAIL (smoke-full-restart): $v did not flush on shutdown"; exit 1; }
done
echo "  all flushed; restarting"
