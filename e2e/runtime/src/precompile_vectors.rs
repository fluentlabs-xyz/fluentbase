//! Differential tests for every precompile guest.
//!
//! Each vector runs twice: natively through the host's `revm::precompile` functions (blst,
//! k256, c-kzg) and as a transaction against the guest module. Both must produce the same output
//! and the same total gas, or both must fail. The BLS12-381 vectors are the EIP-2537 suites in
//! `contracts/bls12381/testcases`; the others are the vectors from the guests' own unit tests plus
//! a few constructed inputs.
//!
//! In the interpreter flavour the heaviest vectors are skipped to keep the run time bounded.
...
/// Interpreter flavour: skip vectors above this native gas to keep the suite under a minute.
const INTERPRETER_GAS_CAP: u64 = 300_000;
...
fn run_case(ctx: &mut EvmTestingContext, case: &Case) {
    ...
    if !cfg!(feature = "wasmtime") {
        if let Ok(out) = &native {
            if out.gas_used > INTERPRETER_GAS_CAP {
                return;
            }
        }
    }
    ...
    match native {
        Ok(out) => {
            assert!(
                result.is_success(),
                "{}: guest failed where the native precompile succeeded: {result:?}",
                case.name
            );
            assert_eq!(
        ...
        Err(err) => {
            assert!(
                !result.is_success(),
                "{}: guest succeeded where the native precompile failed with {err:?}: {result:?}",
                case.name
            );
            assert_eq!(
                result.tx_gas_used(),
