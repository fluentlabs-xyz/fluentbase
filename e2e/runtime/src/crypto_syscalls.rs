...
fn contract_pairing_check_matches_native_and_pays_eip_gas() {
    ...
    assert!(
        (PAIRING_PAIR_GAS..PAIRING_PAIR_GAS + 2_000).contains(&delta),
        "per-pair delta {delta}"
    );
}

/// The rewritten guests import the new syscall family; admission must accept them on every
/// backend this build links, exactly as a runtime upgrade would check them.
#[test]
fn rewritten_guests_pass_system_runtime_admission() {
    use fluentbase_contracts::{
        FLUENTBASE_CONTRACTS_BLS12381, FLUENTBASE_CONTRACTS_BN256, FLUENTBASE_CONTRACTS_ECRECOVER,
        FLUENTBASE_CONTRACTS_EIP7951, FLUENTBASE_CONTRACTS_KZG,
    };
    ...
    use fluentbase_sdk::{
        ...
        PRECOMPILE_BLS12_381_PAIRING, PRECOMPILE_BN256_ADD, PRECOMPILE_BN256_MUL,
        PRECOMPILE_BN256_PAIR, PRECOMPILE_EIP7951, PRECOMPILE_KZG_POINT_EVALUATION,
        PRECOMPILE_SECP256K1_RECOVER,
    };
    let guests: [(&str, &[u8], &[Address]); 5] = [
        (
            "bls12381",
            FLUENTBASE_CONTRACTS_BLS12381.wasm_bytecode,
            &[
        ...
        (
            "bn256",
            FLUENTBASE_CONTRACTS_BN256.wasm_bytecode,
            &[
        ...
        (
            "ecrecover",
            FLUENTBASE_CONTRACTS_ECRECOVER.wasm_bytecode,
            &[PRECOMPILE_SECP256K1_RECOVER],
        ),
        (
            "eip7951",
            FLUENTBASE_CONTRACTS_EIP7951.wasm_bytecode,
            &[PRECOMPILE_EIP7951],
        ),
        (
            "kzg",
            FLUENTBASE_CONTRACTS_KZG.wasm_bytecode,
            &[PRECOMPILE_KZG_POINT_EVALUATION],
        ),
    ];
    for (name, wasm, addresses) in guests {
        for address in addresses {
            validate_system_runtime(wasm, *address)
                .unwrap_or_else(|err| panic!("{name} at {address} is not admitted: {err:?}"));
        }
    }
}
