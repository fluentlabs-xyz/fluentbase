...
fn test_wasm_create() {
    ...
    let value = B256::left_padding_from(&hex!("1000"));
    let gas_limit: u32 = 10_000_000;
    let method_input = WasmCreateMethodInput::new(value.0, wasm_bytecode.to_vec(), gas_limit);
    ...
    println!("wcl contract_address {:x?}", contract_address);
    assert_eq!(expected_contract_address, contract_address);
}
...
fn test_evm_call_from_wasm() {
...

    {
        ...
        test_ctx
            ...
            .set_journal_checkpoint(runtime_ctx.jzkt().unwrap().borrow_mut().checkpoint().into())
            .set_contract_gas_limit(gas_limit.into())
            .set_contract_input_size(
                EVM_CONTRACT_BYTECODE1_METHOD_SAY_HELLO_WORLD_STR_ID.len() as u32
            )
            .set_contract_input(EVM_CONTRACT_BYTECODE1_METHOD_SAY_HELLO_WORLD_STR_ID.into())
