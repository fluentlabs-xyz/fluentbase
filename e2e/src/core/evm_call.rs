...
use fluentbase_types::{address, Address, Bytes, Bytes20, Bytes32, B256, U256};
...
fn evm_create_test() {
    ...
    let created_contract_address = _evm_create(
...
fn evm_call_after_create_test() {
    ...
    let gas_limit: u64 = 10_000_000;
    let created_address = _evm_create(
        &ExecutionContext::DEFAULT,
        EvmCreateMethodInput {
            init_code: EVM_CONTRACT_BYTECODE1.into(),
            value: create_value,
...
fn evm_address_test() {
    ...
    _evm_address(
        &ExecutionContext::DEFAULT,
        address_bytes20_fact.as_mut_ptr(),
    );
