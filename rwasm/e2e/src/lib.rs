...
define_spec_tests! {
    ...
    // fn wasm_fac("fac"); // range end index 1025 out of range for slice of length 1024
    // fn wasm_float_exprs("float_exprs"); // (signal: 11, SIGSEGV: invalid memory reference)

    fn wasm_float_literals("float_literals");

    // fn wasm_float_memory("float_memory"); // (signal: 11, SIGSEGV: invalid memory reference)
