...
fn main_inner(sdk: &mut impl SharedAPI) -> (ExitCode, bytes_or_vecu8::Typ) {
    // read full input data
    let gas_limit = sdk.context().contract_gas_limit();
    let input_length = sdk.input_size();
    let mut input = alloc_slice(input_length as usize);
    sdk.read(&mut input, 0);
    let input = bytes_or_vecu8::copy_from_slice(input);
    // call identity function
    let Ok(result) = revm_precompile::modexp::berlin_run(&input, gas_limit) else {
        return (ExitCode::PrecompileError, bytes_or_vecu8::new());
    };
