...
pub fn process_instruction(
    ...
    // sol_big_mod_exp
    {
        ...
        for (base, exponent, modulus, expected) in test_cases {
            let base = hex::decode(base).expect("failed to decode 'base'");
            let exponent = hex::decode(exponent).expect("failed to decode 'exponent'");
            let modulus = hex::decode(modulus).expect("failed to decode 'modulus'");
            let expected = hex::decode(expected).expect("failed to decode 'expected'");
