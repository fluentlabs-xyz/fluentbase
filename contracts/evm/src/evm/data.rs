...
pub fn data_load<SDK: SharedAPI>(interpreter: &mut Interpreter, _sdk: &mut SDK) {
...
pub fn data_loadn<SDK: SharedAPI>(interpreter: &mut Interpreter, _sdk: &mut SDK) {
    require_eof!(interpreter);
...
pub fn data_size<SDK: SharedAPI>(interpreter: &mut Interpreter, _sdk: &mut SDK) {
    require_eof!(interpreter);
...
pub fn data_copy<SDK: SharedAPI>(interpreter: &mut Interpreter, _sdk: &mut SDK) {
    require_eof!(interpreter);
    gas!(interpreter, VERYLOW);
...
mod test {
    ...
    use revm_interpreter::{
        ...
        primitives::{b256, bytes, Bytecode, Bytes, Eof, PragueSpec},
        DummyHost,
        Gas,
        Interpreter,
    };
