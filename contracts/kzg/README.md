...
KZG commitment precompile-style wrapper (EIP-4844). Exposes blob KZG operations (e.g., point evaluation verification) as host-backed calls.

- Entrypoint: main_entry (routes to the specific op implemented in this crate).
- Input: binary encoding for the requested operation (commitment, proof, point/value). See crate implementation for exact layout.
