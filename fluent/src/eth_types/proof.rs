use crate::hash::{keccak, KECCAK_EMPTY, KECCAK_EMPTY_LIST_RLP, KECCAK_NULL_RLP};
...
impl From<String> for HashBuilderValue {
...
#[cfg(test)]
mod tests {
    use super::AccountProof;

    #[test]
    fn test_account_proof_without_storage_proof() {
        let response = r#"{
        ...
        let val = serde_json::from_str::<AccountProof>(response).unwrap();
        serde_json::to_value(val).unwrap();
        // println!("{:?}", val.balance.as_u64());
    }
}
