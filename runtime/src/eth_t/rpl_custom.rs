...
// // use rustc_hex::ToHex;
// use std::{cell::Cell, cmp, fmt, mem, str};
...
//     header_bytes: &[u8],
//     len_of_len: usize,
// ) -> Result<PayloadInfo, DecoderError> { let header_len = 1 + len_of_len; match
//   header_bytes.get(1) { Some(&0) => return Err(DecoderError::RlpDataLenWithZeroPrefix), None =>
//   return Err(DecoderError::RlpIsTooShort), _ => (), } if header_bytes.len() < header_len { return
//   Err(DecoderError::RlpIsTooShort); } let value_len =
//   decode_usize(&header_bytes[1..header_len])?; Ok(PayloadInfo::new(header_len, value_len))
// }
...
//             Ok(Prototype::Data(_)) => write!(f, "\"0x{}\"", self.data().unwrap().to_vec()),
...
// //         let data =
// // "f84d0589010efbef67941f79b2a056e81f171bcc55a6ff8345e692c0f86e5b48e01b996cadc001622fb5e363b421a0c5d2460186f7233c927e7db2dcc703c0e500b653ca82273b7bfad8045d85a470"
// // .from_hex().unwrap();         let rlp = UntrustedRlp::new(&data);
// //         assert_eq!(format!("{}", rlp), "[\"0x05\", \"0x010efbef67941f79b2\",
