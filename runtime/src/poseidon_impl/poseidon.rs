...
//
pub mod primitives;
// use halo2curves::{group::ff::Field, FieldExt};
...
// // pub trait PoseidonInstructions<F: FieldExt, S: Spec<F, T, RATE>, const T: usize, const RATE:
// // usize>:     Chip<F>
// // {
...
// //         layouter: &mut impl Layouter<F>,
// //         initial_states: &[State<Self::Word, T>],
// //     ) -> Result<Vec<State<Self::Word, T>>, Error> { initial_states .iter()
// .map(|initial_state| //       self.permute(layouter, initial_state)) .collect::<Result<Vec<_>,
// Error>>() //     }
// // }
//
...
// ) -> Result<Squeezing<PoseidonChip::Word, RATE>, Error> { if let Some(input) = input { *state =
//   chip.add_input(&mut layouter, state, input)?; } *state = chip.permute(&mut layouter, state)?;
...
//     const T: usize,
//     const RATE: usize,
// > { chip: PoseidonChip, mode: M, state: State<PoseidonChip::Word, T>, _marker: PhantomData<D>,
// }
//
// impl<
//         F: FieldExt,
...
//         mut layouter: impl Layouter<F>,
//         value: PaddedWord<F>,
//     ) -> Result<(), Error> { for entry in self.mode.0.iter_mut() { if entry.is_none() { *entry =
...
//         mut self,
//         mut layouter: impl Layouter<F>,
//     ) -> Result<Sponge<F, PoseidonChip, S, Squeezing<PoseidonChip::Word, RATE>, D, T, RATE>,
//       Error>
//     {
...
//     pub fn squeeze(&mut self, mut layouter: impl Layouter<F>) -> Result<AssignedCell<F, F>,
// Error> {         loop {
...
//     ) -> Result<AssignedCell<F, F>, Error> { for (i, value) in message .into_iter()
//       .map(PaddedWord::Message) .chain(<ConstantLength<L> as Domain<F,
//       RATE>>::padding(L).map(PaddedWord::Padding)) .enumerate() { self.sponge
//       .absorb(layouter.namespace(|| format!("absorb_{i}")), value)?; } self.sponge
