use crate::{
    ...
    gadgets::binary_number::{AsBits, BinaryNumberChip, BinaryNumberConfig},
    impl_expr,
    state_circuit::{
...
struct Queries<F: Field> {
    tag: Query<F>,       // 4 bits
    field_tag: Query<F>, // 8 bits, so we can pack tag + field_tag into one limb.
...
impl<F: Field> Queries<F> {
    fn new(meta: &mut VirtualCells<'_, F>, keys: SortKeysConfig, rotation: Rotation) -> Self {
        ...
        Self {
            ...
            field_tag: keys.field_tag.rotation(rotation.0),
