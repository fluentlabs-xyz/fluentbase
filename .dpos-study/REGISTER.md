...
|---|---|---|---|---|---|---|
| K-1 | SERIOUS | governance через `setBlsVerifier` + `setSlashFundAddress` получает право tombstone'ить любого валидатора и забирать самостейк | `config.rs`, `consensus.rs` | Э1 / С+ — КB-4 убирает слэш по улике; сеттер верификатора после init удалить (1.3); остаток — поддельный PoP при регистрации (дыра §6) | закрыта — `f70ceffe` (1.10: верификатор инлайн, `setBlsVerifier` удалён). Остаток: подменяемость сужена до механизма обновления предеплоев `runtime-upgrade` (R10.1) | history/PLAN.md 1.10; history/E1-REFLECTION.md R10.1 |
...
| K-13 | MINOR | ABI-декодер молча усекает целые и игнорирует хвост | `util.rs`, SDK `crates/codec` | Э1 / П — строгий декодер в SDK (1.6) | отложена — 1.6 (строгий ABI-декодер в SDK `crates/codec`), открыта | history/PLAN.md 1.6 |
...
| K-16 | MINOR | `initialize` принимает cap 1..3 — первый коммит всегда ревёртит | `config.rs` | Э1 / П — cap ≥ 4 при init (1.6) | открыта частично — гейт `cap < MIN_COMMITTEE_LENGTH` есть в сеттере `set_active_validators_length`, но не в `validate_initialization` (1.6, проверено 09-08); Э0.2 её не закрыл вопреки плану | history/PLAN.md 1.6; history/E0-LOG.md |
...
| K-18 | MINOR | view без `ensure_initialized` падают `IntegerDivisionByZero` | `staking.rs` | Э1 / П — 1.6 | открыта частично — 1.5 удалила часть вьюх; `ensure_initialized`: 21 вызов в `staking.rs`, ноль в `config.rs` (1.6, 09-08). Столбец «Закрывает» 1.5 расходится с 1.6 (отчёт §4) | history/PLAN.md 1.5, 1.6 |
