...
- **Статус 2026-09-14 (Э5 5.3 заход Б — ЗАКРЫТА в коде и поведении, стенд):** идентичность лога `(dealer, hash)` (`LogId`), одна функция записи `DkgCeremony::insert_log` на seal/gossip/resolver/replay; второй валидный лог того же дилера пишется РЯДОМ, пара — `equivocations` + `JournalRecord::DealerEquivocation(first, second)` (tag 4), WARN + `dpos_dkg_dealer_equivocation_total`; локальный бан только на gossip-`Reveal`, целевой фетч по точному хэшу бан не применяет (так пришпиленный лог банённого дилера доходит до жертвы); refetch по пришпиленному хэшу (`fetch_missing_logs` — `(dealer, hash)` из `agreed_pinned`, не `holds()`; `dkg_agree::fetch_bodies` — с хэшем предложения); wire `DkgLogKey {epoch, dealer, hash}` 72 байта. **Оба стенд-теста сменили вердикт:** `a_two_log_dealers_victim_refetches_the_pinned_log_and_keeps_its_share` (`heights=[72;4]`, `equivocations=[1,0,0,0]`), второй — цепь больше НЕ останавливается (жертва с share ⇒ 3 честных = quorum(4); прежняя остановка была следствием первого звена, не «граница I5»). Ревью: `history/E5-3-B-DSH-1.md`, `-DSH-2.md`, `E5-3-B-REVIEW.md` (вердикт КОММИТИТЬ). Мутация first-wins воспроизведена: приёмка красная. Он-чейн ничего не шлётся (Д-6 defer). Остатки, не этой записи: never-acked игрок может взять dealing второго полинома до доказательства эквивокации — share теряется, форка нет (dsh E-10; заход А); **live-`adopt_share` самопроверяет share против локального `out`, не против `group_key` артефакта** (финальное ревью F-02, было и на HEAD) — см. R-039.
...
- **Статус 2026-09-13 (строка Э5 5.2): НЕ ЗАКРЫТА, и попытка закрытия ОТКАЧЕНА** (отклонение 5.2а-Д-5). Строка плана 5.2 предписывала сделать порог абсолютным после того, как σ-hold перестанет зависеть от re-jump. Предусловие выполнено (`Pending` в `SeedIndex` + settle по ребру `KeyAvailable`), но оно НЕПОЛНОЕ: re-jump нёс вторую работу — выход для узла с остановленным ИСПОЛНЕНИЕМ, — и 5.2 её ничем не заменила. Оркестратор воспроизвёл клин живым прогоном: мутация `testbed/preconditions.rs:353` на продакшн-форму `Some(JUMP_THRESHOLD)` даёт `heights=[264, 264, 264, 95] jumps=[] marshal_fin=[…, Some(159.0)]`, `missing=[160…264]`, паника `preconditions.rs:411: node 3 did not climb out by repeated re-jumps: []` — числа совпали с замером, записанным в доккомментарии того же теста (`:303-315`: «A gate at or above `2·interval` therefore wedges an execution-stalled node PERMANENTLY after §5.2»). Арифметика: достижимый разрыв `tip − fin < 3·interval` и равен `2·interval`, когда `fin` стоит на терминале эпохи, — ниже абсолютного порога 1024 при любом `interval < 512`, значит прыжок не арминуется. Настоящее предусловие R-016 шире того, что называет строка плана: нужен ВТОРОЙ выход для execution-stalled узла. Тот же класс, что R-131: снятая опора при непересчитанном основании.
...
- **Статус 2026-09-15 (Э5 5.3 заход А1 — явный автомат `EpochState`, журнал `history/E5-3-A1.md`, ревью `history/E5-3-A1-DSH-{1A,1B,2A,2B}.md`, финал `history/E5-3-A1-REVIEW.md` — КОММИТИТЬ) — ЗАКРЫТА В КОДЕ (с оговоркой):** потеря тела ⇒ сигнал `AgreementConfig::body_lost` (канал лаунчер → актор) ⇒ `Sealed → Acquiring(ArtifactForCeremony)` + `Stalled{BodyLost}` + pull сразу; сигнал `try_send`-lossy, но актор сам переводит `Sealed` в `Acquiring` на границе (`E ≤ now`) и стор читается первым шагом каждого тика (`reconcile_with_store`). Остаток (финал F-02 LOW): сигнал, пришедший пока эпоха ещё `Dealing`, сбрасывается — heal откладывается до границы, паритет с HEAD.
...
- **Статус 2026-09-15 (Э5 5.3 заход А1 — явный автомат `EpochState`, журнал `history/E5-3-A1.md`, ревью `history/E5-3-A1-DSH-{1A,1B,2A,2B}.md`, финал `history/E5-3-A1-REVIEW.md` — КОММИТИТЬ) — ЗАКРЫТА В КОДЕ:** `Torn`/`NoFile` одним гейтом по дедлайну в `recover(E)`: `h < seal ⇒ Dealing` (reconstruct), `h ≥ seal ⇒ SatOut{key}` (терминал), `start_fresh` без гейта не существует; возвращена фикстура нулевого окна (`INTERVAL = 20`), в которой NoFile на дедлайне даёт `sat_out`, не старт; мутация «NoFile всегда старт» — красная.
...
- **Статус 2026-09-15 (Э5 5.3 заход А1 — явный автомат `EpochState`, журнал `history/E5-3-A1.md`, ревью `history/E5-3-A1-DSH-{1A,1B,2A,2B}.md`, финал `history/E5-3-A1-REVIEW.md` — КОММИТИТЬ) — ЗАКРЫТА В КОДЕ:** heal идёт раз на набор входов (`RecomputeState.attempted`, ставится после успешной загрузки входов); `NoFile`/`Torn`/`Err` ⇒ `Stalled{HealFailed}` (защёлка + gauge), повтор по новому телу; off-poly при пустом `want` ⇒ `Unrecoverable{key}`; тест — счётчики не растут на тиках.
...
- **Статус 2026-09-14 (Э5 5.3-Б, финальное ревью F-02 — ПЕРЕОТКРЫТА НАПОЛОВИНУ, проверено оркестратором по коду):** `validate_share_on_poly` на live-пути стоит, но проверяет share против ЛОКАЛЬНОГО `out` (`drive_finalization` → `adopt_share(e, &committee, out, share)`, `beacon/actor.rs:1783`; то же на HEAD `18573e76:1697`), тогда как heal — против ПРИШПИЛЕННОГО outcome (`:2716-2719`). Под мутацией «`scoped_pinned_logs` по дилеру» жертва приняла share над непришпиленным набором с `ok=1`, без demote и ERROR. Второй заслонки нет. Вне рамки 5.3-Б (было на HEAD); правка одна — гейт live-adopt против `group_key` артефакта. Кандидат в отдельный тикет; в Э5 — заход А строки 5.3 (там `adopt_share` и так переписывается) или Всплыло.
- **Статус 2026-09-15 (Э5 5.3 заход А1 — явный автомат `EpochState`, журнал `history/E5-3-A1.md`, ревью `history/E5-3-A1-DSH-{1A,1B,2A,2B}.md`, финал `history/E5-3-A1-REVIEW.md` — КОММИТИТЬ) — ЗАКРЫТА ЦЕЛИКОМ:** один гейт `share_on_artifact` (`beacon/actor.rs:1762`, открыл сам) на ВСЕХ путях в `Keyed` (live finalize, heal, `ArtifactForShare`+артефакт, `recover` share∧artifact) — доля проверяется против `group_key` ПРИШПИЛЕННОГО артефакта (`StoredArtifact.held`/`DkgProposal`), локальный `out` — только источник доли; отказ ⇒ `drop_share` (RAM + файл) + `Acquiring(Logs)` + `Stalled{OffPolynomial}`. Мутация «гейт против локального `out`» — красная; финальный ревьюер снял гейт на одном пути — красные ровно два теста.
...
- **Статус 2026-09-15 (Э5 5.3 заход А1 — явный автомат `EpochState`, журнал `history/E5-3-A1.md`, ревью `history/E5-3-A1-DSH-{1A,1B,2A,2B}.md`, финал `history/E5-3-A1-REVIEW.md` — КОММИТИТЬ) — ЗАКРЫТА В КОДЕ:** `Torn` до seal ⇒ `Dealing` с reconstruct (ничего не отправлено), после seal ⇒ `SatOut` (терминал с памятью, не повтор каждый тик); четыре клетки {Torn, NoFile} × {до, после seal} — юниты `recover_*`.
...
- **Статус 2026-09-15 (Э5 5.3 заход А1 — явный автомат `EpochState`, журнал `history/E5-3-A1.md`, ревью `history/E5-3-A1-DSH-{1A,1B,2A,2B}.md`, финал `history/E5-3-A1-REVIEW.md` — КОММИТИТЬ) — НЕ МЕНЯЛАСЬ:** резолверные каналы не трогались; остаётся открытой (А2/Э6).
...
- **Статус 2026-09-15 (Э5 5.3 заход А1 — явный автомат `EpochState`, журнал `history/E5-3-A1.md`, ревью `history/E5-3-A1-DSH-{1A,1B,2A,2B}.md`, финал `history/E5-3-A1-REVIEW.md` — КОММИТИТЬ) — ЗАКРЫТА ЧАСТИЧНО:** фазы эпох теперь наблюдаемы через `EpochState` (13 вариантов) и gauge `dpos_dkg_stalled{reason}` (девять причин, защёлка на (эпоха, причина), снимается при смене фазы); `dpos_dkg_artifact_conflict_total`, `dpos_dkg_conflict_marker_failed_total`, `dpos_dkg_artifact_handoff_lost_total`. Остаток: возраст отложенного fin, размеры карантина σ — вне А1.
...
- **Статус 2026-09-15 (Э5 5.3 заход А1 — явный автомат `EpochState`, журнал `history/E5-3-A1.md`, ревью `history/E5-3-A1-DSH-{1A,1B,2A,2B}.md`, финал `history/E5-3-A1-REVIEW.md` — КОММИТИТЬ) — ЗАКРЫТА В КОДЕ:** см. R-051 — `Torn` первого рекорда до дедлайна ⇒ `Dealing` с reconstruct.
...
- **Статус 2026-09-11:** закрыта — `65a23db7` (Э4 4.1 Б2: `soft_enter_span`, `register_soft_entered`, `EpochSchemeProvider::register` и второй кэш `CertInlet.schemes` удалены; чтение записи через модуль И ЕСТЬ регистрация схемы — `EpochEntry{record, scheme}`, одна карта; `register_span` останавливается на первой нечитаемой ЗАПИСИ). Стенд `four_nodes_at_four_heights_hold_one_committee_record_per_epoch` (`03c3151f`; с 09-14 — `five_nodes_at_five_heights_…`, см. Ex-4.1c) пинует одну запись на эпоху у четырёх узлов на четырёх якорях. _Источник:_ history/E4-1-B2.md §0.1–0.2, E4-1-B3.md В§0.5
...
- **Статус 2026-09-14 (Э5 5.3-В+Г1) — ЗАКРЫТА ЗАПИСЬЮ, окно не расширено:** одна функция окна `within_ingress_window` с `INGRESS_LOOKAHEAD_EPOCHS = 2` (три бывшие копии — один сайт, `beacon/actor.rs:124-162`); `+3` не добавлен сознательно: часы актора — max по трём фидерам (включая ordering-tip), отставание на ≥2 эпохи по всем трём — случай замёрзшего tip (cold-start); дилинги лечит ретрансмит, reveal — refetch по хэшу (5.3-Б), confirm — re-mint по росту ширины. **Остаток, пред-существующий с 4.3-А (dsh E-01 круга 2 В+Г1):** `ShareConfirm` НОВОГО члена, ещё не входящего в три записи окна получателя, режется пред-декодным гейтом как `secondary`/`untracked` и не переиздаётся (`mint` edge-triggered, `confirmations.rs:186-188`); лечится только через предложение лидера, если тот его получил. Фикс — переиздание `mint` при росте окна; вне 5.3.
...
- **Статус 2026-09-14 (Э5 5.3-В+Г1) — ЗАКРЫТА В КОДЕ, с пересмотром формулировки:** «один источник» = классификация отправителя ТОЛЬКО пред-декодным `GatedReceiver` по `TrackedWindow` (в проде с 4.3-А, в стенде — впервые: окно на узел через `TrackSink` продакшн-`assemble_tracked_peers`, `with_tombstones` над стендовым `TombstoneSet`, заполняемым `observe(&snapshot)`); в акторе по-эпошной проверки членства НЕТ (`beacon_member` удалён), эпоху к отправителю привязывает ПОТРЕБИТЕЛЬ (commonware `Info` для дилингов/ack, ростер `committee_for` для confirm и при буферизации/дрейне/диспатче — `DkgCeremony::has_seat`), отказ там виден: `refuse(.., "no_seat")` — единственный сайт `record_ingress_drop` в биконе. Первая форма (членство по окну в акторе) была реализована и ОТВЕРГНУТА dsh-ревью (`history/E5-3-VG1-DSH-1.md` D-01/D-05): окно ET ключуется по `fin`, часы актора опережают — кадры текущей церемонии резались бы. **Остаток (dsh E-02):** гейт по окну ET и потребитель по `committee_for` могут разойтись на пропущенной несовершённой соседней записи или при опережении часов — пред-существует с 4.3-А, расхождение теперь видно меткой `no_seat`, не молча.
...
- **Статус 2026-09-14 (Э5 5.2 заход В): ФОРМУЛИРОВКА ВЫШЕ НЕВЕРНА В ДВУХ МЕСТАХ, обе поправки проверены оркестратором по чекауту commonware.**
  1. **Механизм.** «`provider.scoped(epoch) == None` ⇒ `send_lossy(false)`» верно НЕ для того пути, который назван. При отсутствующей схеме НА ДЕКОДЕ маршал пишет `"ignoring stale delivery"` и отвечает `response.send_lossy(**true**)` (`marshal/core/actor.rs:965-971`), то есть пира не наказывает; наш `EpochSchemeProvider` (`outer.rs:283-290`) переопределяет только `scoped`, а дефолт `all()` возвращает `None` (`cryptography/src/certificate.rs:417-419`), поэтому на декоде срабатывает именно эта ветка. Форма с `false` живёт ТОЛЬКО внутри `verify_delivered` (второе чтение провайдера) и требует схемы, которая ЕСТЬ при декоде и отсутствует при проверке. Обе формы названы в `.claude/COMMONWARE_INTERNALS.md:236`; в эту запись попала вторая, а приписана была первой.
  2. **Последствие.** «Исключает честного пира навсегда» — ПРАВДА, но не на транспорте. На `deliver == false` резолвер делает ДВА действия (`resolver/src/p2p/engine.rs:437-438`): `block!(self.blocker, …)` — транспортный бан, и он у нас обезврежен, потому что все три конструкции `OuterBuilder` дают `NoopBlocker` (`dpos.rs`, стенд); и `self.fetcher.block(peer)` — от выбора `Blocker` НЕ зависящее, кладёт пира в `excluded` (`fetcher.rs:510-517`), множество, из которого никогда не удаляют (вставка `:516`, чтение фильтром `:242`, длина `:567`). Узел больше НИКОГДА не спросит этого пира по этому каналу. Наблюдаемая — гейдж `peers_blocked` = `fetcher.len_blocked()` (`engine.rs:176-178`).
- **Что заход В ЗАКРЫЛ (стенд-тест `an_epoch_outside_this_nodes_read_window_costs_no_peer_its_channel`, `testbed/cert_inlet_tests.rs`):** в прогоне, где нечитаемая эпоха достоверно случается (свидетели: `defers > 0` при `rotations = 0` у инлета жертвы и ненулевой `dpos_frontier_dropped_total`), НИ ОДИН пир не исключён — пять свидетелей: счётчик `BlockerSpy` в обоих слотах, гейдж `peers_blocked` на ВСЕХ 18 семействах резолверов (включая beacon/dkg, которых шпион не видит; ноль = «на момент последней итерации движка»), WARN самого макроса `block!` из `Outcome::logs` (безоконный), пустое множество меток `dpos_frontier_rejected_total`, `deliveries_rejected == 0` по узлам (`CountingHandler`, не зависит ни от слота, ни от `Self::reject`). Покрытие собирается из рекордера (мутация «новая метка» красит тест — воспроизведено оркестратором). Исполняется при этом правило ФРОНТИРА: шаг (5) `plane_upstream::deliver` (`:486-490`) дропает ответ и возвращает `true` на четырёх причинах «это моя проблема» (`no_geometry`, `out_of_window`, `not_readable`, `read_failed`). Сужение названо честно: фикстура производит ровно `out_of_window`, покрытие сравнивается ТОЧНЫМ равенством с `COVERED`, три остальные причины видны нулями. Цена повтора измерена: 31 выброшенный раунд-трип за 159.916 с виртуального времени, спина нет.
- **Что ОСТАЁТСЯ ОТКРЫТЫМ:** арм `verify_delivered ⇒ false` резолвера МАРШАЛА стендом не исполняется, и препятствие выше нашего кода (см. поправку 1) — фикстуры для него нет, закрытие возможно лишь разбором либо другой фикстурой («якорь ниже высоты коммита ⇒ `NotReadable`», отдельная строка). Отдельно: тест пинует СТЕНД, а не выбор blocker'а в продакшне — замена `OuterBuilder.blocker` на настоящий его не покраснит. Обоснование `NoopBlocker` в `dpos.rs` написано про симплекс-батчер, маршальской половины в нём нет; дописать — отдельная правка. _Источник:_ history/E5-2-V.md; history/E5-2-V-REVIEW.md; history/E5-ORCHESTRATOR.md К-89, К-114…К-120
...
### R-131 · SERIOUS · Follower с reth ниже блока активации паркуется навсегда: арм `Some` остался без привода EL после 4.2-Б2

**Механизм.** `launch_follower` (`consensus/dpos.rs:2916`) выбирает вход по `read_geometry(&reader, rf_hash)`. В арме `Some((activation, interval))` (`:3012-3035`) привода EL нет ни одного: `mk_el_sync` (`:3000`) используется ТОЛЬКО в арме `None` (`:3054`, `:3078`). При `rf_num < activation` вызывается `wait_for_activation_block` (`:280-320`) — чистый опрос `provider.block_hash(activation)` + `ctx.sleep(2s)`, без FCU, без таймаута, без обращения к апстриму. Блоки `1..activation−1` — до-DPoS секвенсерные: сертификатов на них не существует, в marshal-архивах их нет, лестница и прыжок их не достают, а reth без FCU на неизвестную голову бэкфилл не арминует. Узел стоит вечно.

**Провенанс — регрессия, не пробел.** `git show dacd1bfa^:crates/dpos/consensus/src/dpos.rs` (проверено оркестратором): арм `Some` начинался с `let el = mk_el_sync(activation);` и `Some(latest) => el.sync_to(&latest).await?`, а `wait_for_activation_block` стоял РЕЗЕРВНЫМ армом на `get_latest() == None` и на `upstream.is_none()`. Заход 4.2-Б2 снял основной путь и оставил резерв единственным. Проект требовал другого: §5.2 (`history/E4-CORE-DESIGN.md:514`) — «`sync_to` — единственный писатель EL-тегов на этом пути, и это не меняется; меняется только его ВХОД»; `:516` — «Первые два идут через `Frontier` (у follower'а с datadir геометрия и модуль читаются локально на `rf_hash`)». Написано «пропустить вход через проверенную цель», реализовано «снять вход».

**Достижимость.** Девнет: свежий `--cert-follow` против уже свопнутой цепи — воспроизведено оркестратором ДВАЖДЫ подряд (`make smoke-cert-follow`: `connected_peers=1`, `latest_block=0`, `heights=[…, 95]`, `FAIL … did not align with v0 past 97`). Прод: любой узел, остановленный до активации и поднятый после, чей `rf_hash` несёт читаемый `ChainConfig`. Валидаторская половина — арм `FreshMigration`, тем же предикатом.

**Уверенность.** `[KNOWN]` — живой прогон ×2 плюс `dacd1bfa^` плюс текст §5.2, всё открыто оркестратором.

**Ход по плану.** Строка **4.4** — follower-половина. Валидаторская половина остаётся открытой после неё.

**Связано.** E4-30 (третий `sync_to`), E4-05 (follower без checkpoint), Д-2, R-016 (тот же класс «снятая опора, не пересчитанное основание»). Разборы: `history/E5-COLDSTART-RESEARCH.md`, `history/E5-COLDSTART-COUNTER.md`, `history/E5-COLDSTART-DECISION.md`.

- **Статус 2026-09-14 (Э5 строка 4.4): FOLLOWER-ПОЛОВИНА ЗАКРЫТА кодом, живой приёмкой ещё нет.** Арм `Some` получил марш из пяти входов (`dpos.rs::follower_entry` ⇒ `FollowerEntry::{Local, Checkpoint, WaitLocal, ChainBelowActivation, Certificate{target}}`), решение вынесено в чистую функцию и покрыто 15 юнитами; `wait_for_activation_block` выжил как ОДИН из пяти шагов, а не как весь арм. Закрытие проверено оркестратором по коду, не по реляции (К-81, К-84, К-92…К-103). По дороге найдены и закрыты в ПРИЧИНЕ два дефекта, которых в постановке не было: (1) актор WS-апстрима не читал мейлбокс ни в одной фазе без соединения, поэтому `get_latest` не возвращался никогда, марш не проворачивался и `block_hash(activation)` не перечитывался — то есть первая версия правки ВОССТАНАВЛИВАЛА ровно тот класс отказа, который R-131 и описывает (D-01, BLOCKER; закрыто `while_disconnected`); (2) якорь `rf_hash` пиновался до цикла, поэтому половина «ни одна эпоха не читается» не лечилась поллингом, хотя её собственный `warn!` обещал «Polling (no give-up)» (D-02). **ОСТАЁТСЯ ОТКРЫТЫМ:** валидаторская половина (`FreshMigration`) — не делалась; арм не покрыт ни одним тестом, потому что `launch_follower` не строит ни стенд, ни юниты (D-08), и правки D-02/D-04 держатся на чтении кода; **живая приёмка ВЫПОЛНЕНА 2026-09-14 — все три кейса зелёные (прогоны оркестратора), то есть follower-половина закрыта теперь и в ПОВЕДЕНИИ:** `smoke-cert-follow` дал фазу 1 verbatim `cert-follow: entered below the activation block by an authenticated certificate target=72 landing=69 activation=64` — первое живое исполнение нового арма; `smoke-cert-cascade` дал арму `Checkpoint` свидетеля на ОБОИХ исходах (годный checkpoint ⇒ посадка, негодный ⇒ fail-closed отказ `'could not be obtained from any peer'`, а не молчаливый парк) и снял опасение отклонения `4.4а-Д-7`; `smoke-cert-keyless` — обе фазы. Но армы `WaitLocal` и `ChainBelowActivation`, а также правки D-01 (мёртвый апстрим) и D-02 (поздний `commitEpochCommittee`) живых свидетелей НЕ имеют: эти конфигурации ни один из трёх кейсов не создаёт. _Источник:_ history/E4-4-A.md; history/E4-4-A-REVIEW.md; history/E5-ORCHESTRATOR.md