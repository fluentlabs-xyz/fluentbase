...
|---|---|---|---|---|---|---|
| K-1 | SERIOUS | governance через `setBlsVerifier` + `setSlashFundAddress` получает право tombstone'ить любого валидатора и забирать самостейк | `config.rs`, `consensus.rs` | Э1 / С+ — КB-4 убирает слэш по улике; сеттер верификатора после init удалить (1.3); остаток — поддельный PoP при регистрации (дыра §6) | закрыта — `f70ceffe` (1.10: верификатор инлайн, `setBlsVerifier` удалён) и `b237a81e` → `0f283a82` (1.3, 09-11: таймлок 7 эпох «заявить → применить» на `setBlendReserve`, с отзывом и сроком годности заявки; `setSlashFundAddress` из-под таймлока выведен контр-ревью — он путь ремонта для K-22, `DECISIONS.md` §3). Остаток: подменяемость сужена до механизма обновления предеплоев `runtime-upgrade` (R10.1) | history/PLAN.md 1.10; history/E1-REFLECTION.md R10.1 |
...
| K-22 | MINOR | отказ фонда оставляет конфискат на контракте без пути вывода; событие `seized = 0` | `consensus.rs` | Э1 / П — 1.6 | закрыта — `3c4560db` (1.6, 09-11): отказ фонда ревёртит всю конфискацию `ERR_STAKING_TOKEN_CALL_FAILED`, тумбстоун/тюрьма/удаление из активных/штамп невидимости откатываются вместе. Принятая цена — `DECISIONS.md` §3 «K-22» | history/E1-CONTRACT-2.md §2 П3 |
