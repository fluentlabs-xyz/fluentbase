...
Причина одна: маршрут улик покупает досягаемость ценой 2741 строки, из которых 744 —
...
**Поправка 2026-09-11 (`E1-CONTRACT-REVIEW.md` F-12).** Выше стояло «8 эпох досягаемости», и это
уже собственной находки §5 ответ 1: `evm.rs:1164` подставляет `current_epoch` безусловно, поэтому
системный маршрут не покрывает ничего, кроме текущей эпохи, и теряется не хвост эпохи и не
глубина, а любая улика, чей держатель не попал в лидеры ДО конца эпохи, плюс любая пара,
собравшаяся после границы. Это потеря по РОЛИ и по МОМЕНТУ. «8 эпох» — ширина окна WAL-маршрута
(§5 ответ 5), то есть верхняя граница того, как долго уже собранный заряд ещё носят, а не то,
что вариант (А) отнимает. Рекомендация от поправки не меняется: цена (А) выше, чем её называла
заголовочная фраза, а довод против маршрута — 2741 строка и два непокрытых шва — от глубины окна
не зависит.
...
- **слэшинг перестаёт быть permissionless** (дописано 2026-09-11, `E1-CONTRACT-REVIEW.md` F-13).
  Три удаляемых селектора — единственная точка контракта без гейта на вызывающего вообще (§1.2,
  «гейт на вызывающего: НЕТ НИ ОДНОГО»); после (А) наказать эквивокатора может только комитет
  через предысполнительный системный вызов. Это отдельное записанное свойство проекта
  («equivocation tombstone, permissionless v1»), и (А) его отменяет — в списке потерь выше оно
  не называлось ни разу, хотя §4(В) упоминает стороннего подателя как довод против половинчатого
  варианта. Насколько владелец считает permissionless-слэшинг обязательным, из дерева не читается
  — это вопрос к решению, а не находка;
