...
use alloy_sol_types::sol_data::{Address, Bytes};
use alloy_sol_types::{sol, SolCall, SolEvent, SolType, SolValue};
use fluentbase_sdk::{
    ContextReader, ExecutionContext, JzktAccountManager, LowLevelAPI, LowLevelSDK, U256,
};
use hex_literal::hex;
...
mod test {
    ...
    fn with_test_input<T: Into<Bytes>>(input: T, caller: Option<Address>) {
        ...
        contract_input.contract_input = input.into();
