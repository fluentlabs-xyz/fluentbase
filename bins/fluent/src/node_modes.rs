...
/// `--cert-follow` / `--cert-upstream` / `--dpos.staking-config`. The `Default`
...
pub(crate) struct ResolvedModes {
    ...
    /// full node (the standalone bare `--cert-upstream` path stays `consensus_url` →
...
pub(crate) fn resolve_node_modes(
    ...
    // using the FIRST entry of the (repeatable) --cert-upstream list (D3).
    if let Some(cert_upstream) = ext.cert_upstream.first() {
        modes.consensus_url = Some(cert_upstream.clone());
    ...
    // (clap) guarantees both `--cert-upstream` and `--dpos.staking-config` are
    // present. Clear `consensus_url` so `launch_consensus_node` does not also run.
    if ext.cert_follow {
        assert!(
            !ext.cert_upstream.is_empty(),
            "requires_all guarantees --cert-upstream"
        );
