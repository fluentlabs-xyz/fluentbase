    ...
    if let Some(path) = &ext.dpos_cfg.dpos_staking_config {
        match fluentbase_staking_reader::reader::StakingReaderConfig::from_json_path(path) {
            Ok(parsed) => {
                ...
                modes.staking_reader_cfg = Some(parsed.clone());
                // Both-or-neither: the committee-commit gate (evm.rs:870) needs
                // BOTH addresses non-zero. A one-zero typo would silently
                // downgrade the node to plain Ethereum and wedge at the first
                // epoch boundary with no error — fail loud at load instead.
                if modes.staking_address.is_zero() != modes.chain_config_address.is_zero() {
                    eprintln!(
                        "--dpos.staking-config partial config: staking_address \
                         ({}) and chain_config_address ({}) must be BOTH zero \
                         (non-DPoS) or BOTH non-zero (DPoS)",
